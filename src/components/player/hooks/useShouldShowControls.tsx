...
export function useShouldShowControls() {
  ...
  const lastHoveringState = usePlayerStore((s) => s.interface.lastHoveringState);
  const isPaused = usePlayerStore((s) => s.mediaPlaying.isPaused);
  const hasOpenOverlay = usePlayerStore((s) => s.interface.hasOpenOverlay);
  const isHoveringControls = usePlayerStore((s) => s.interface.isHoveringControls);
  ...
  const showTargetsWithoutPause = isHovering || (isHoveringControls && !isUsingTouch) || hasOpenOverlay;
  const showTargetsIncludingPause = showTargetsWithoutPause || isPaused;
  const showTargets = isUsingTouch ? showTargetsWithoutPause : showTargetsIncludingPause;
