...
export function Section(props: { children: React.ReactNode; className?: string }) {
  return <div className={classNames("pt-4 space-y-1", props.className)}>{props.children}</div>;
}

export function ScrollToActiveSection(props: { children: React.ReactNode; className?: string; loaded?: boolean }) {
  const scrollingContainer = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const active = scrollingContainer.current?.querySelector("[data-active-link]");
    ...
    scrollingContainer.current?.scrollTo(0, activeYPos - boxRect.height / 2 + activeLinkRect.height / 2);
  }, [props.loaded]);

  return (
    <div ref={scrollingContainer} className={classNames("pt-4 space-y-1", props.className)}>
      {props.children}
    </div>
  );
}
