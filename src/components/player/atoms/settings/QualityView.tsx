...
import { SourceQuality, allQualities, qualityToString } from "@/stores/player/utils/qualities";
...
export function QualityView({ id }: { id: string }) {
  ...
  const enableAutomaticQuality = usePlayerStore((s) => s.enableAutomaticQuality);
  ...
  return (
    <>
      <Menu.BackLink onClick={() => router.navigate("/")}>{t("player.menus.quality.title")}</Menu.BackLink>
      <Menu.Section className="flex flex-col pb-4">
        {visibleQualities.map((v) => (
          <SelectableLink
            key={v}
            selected={v === currentQuality}
            onClick={availableQualities.includes(v) ? () => change(v) : undefined}
            disabled={!availableQualities.includes(v)}>
            {qualityToString(v)}
          </SelectableLink>
        ))}
        <Menu.Divider />
        <Menu.Link rightSide={<Toggle onClick={changeAutomatic} enabled={autoQuality} />}>{t("player.menus.quality.automaticLabel")}</Menu.Link>
        <Menu.SmallText>
          <Trans i18nKey={isIosHls ? "player.menus.quality.iosNoQuality" : "player.menus.quality.hint"}>
            <Menu.Anchor onClick={() => router.navigate("/source")}>text</Menu.Anchor>
          </Trans>
        </Menu.SmallText>
      </Menu.Section>
    </>
