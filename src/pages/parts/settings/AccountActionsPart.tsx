...
export function AccountActionsPart() {
  ...
  return (
    <div>
      ...
      <SolidSettingsCard paddingClass="px-6 py-12" className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12">
        <div>
          <Heading3>{t("settings.account.actions.delete.title")}</Heading3>
          <p className="text-type-text">{t("settings.account.actions.delete.text")}</p>
        </div>
        <div className="flex justify-start lg:justify-end items-center">
          <Button theme="danger" loading={deleteResult.loading} onClick={deleteModal.show}>
      ...
      <Modal id={deleteModal.id}>
        <ModalCard>
          <Heading2 className="!mt-0">{t("settings.account.actions.delete.confirmTitle")}</Heading2>
          <Paragraph>{t("settings.account.actions.delete.confirmDescription")}</Paragraph>
          <Button theme="danger" loading={deleteResult.loading} onClick={deleteExec}>
