  ...
  return (
    <SettingsCard>
      <div className="flex justify-between items-center gap-4">
        <div className="my-3">
          <p className="text-white font-bold mb-3">{t("settings.connections.workers.label")}</p>
          <p className="max-w-[20rem] font-medium">
            <Trans i18nKey="settings.connections.workers.description">
              <MwLink to="https://movie-web.github.io/docs/proxy/deploy">Proxy documentation</MwLink>
            </Trans>
          </p>
        </div>
        <div>
          <Toggle onClick={() => setProxyUrls((s) => (s === null ? [] : null))} enabled={proxyUrls !== null} />
        </div>
      </div>
      {proxyUrls !== null ? (
        <>
          <Divider marginClass="my-6 px-8 box-content -mx-8" />
          <p className="text-white font-bold mb-3">{t("settings.connections.workers.urlLabel")}</p>

          <div className="my-6 space-y-2 max-w-md">
            {(proxyUrls?.length ?? 0) === 0 ? <p>{t("settings.connections.workers.emptyState")}</p> : null}
            {(proxyUrls ?? []).map((v, i) => (
              <div
                ...
                <AuthInputBox value={v} onChange={(val) => changeItem(i, val)} placeholder={t("settings.connections.workers.urlPlaceholder") ?? undefined} />
                <button
                  type="button"
...
function BackendEdit({ backendUrl, setBackendUrl }: BackendEditProps) {
  ...
  return (
    <SettingsCard>
      <div className="flex justify-between items-center gap-4">
        <div className="my-3">
          <p className="text-white font-bold mb-3">{t("settings.connections.server.label")}</p>
          <p className="max-w-[20rem] font-medium">
            <Trans i18nKey="settings.connections.server.description">
              <MwLink to="https://movie-web.github.io/docs/backend/deploy">Backend documentation</MwLink>
            </Trans>
          </p>
        </div>
        <div>
          <Toggle onClick={() => setBackendUrl((s) => (s === null ? "" : null))} enabled={backendUrl !== null} />
        </div>
      </div>
      {backendUrl !== null ? (
        <>
          <Divider marginClass="my-6 px-8 box-content -mx-8" />
          <p className="text-white font-bold mb-3">{t("settings.connections.server.urlLabel")}</p>
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        <SetupPart />
        <ProxyEdit proxyUrls={props.proxyUrls} setProxyUrls={props.setProxyUrls} />
        <BackendEdit backendUrl={props.backendUrl} setBackendUrl={props.setBackendUrl} />
      </div>
    </div>
  );
}
