...
import { CaptionSetting, ColorOption, colors } from "@/components/player/atoms/settings/CaptionSettingsView";
...
export function CaptionPreview(props: { fullscreen?: boolean; show?: boolean; styling: SubtitleStyling; onToggle: () => void }) {
      ...
      <Transition animation="fade" show={props.show}>
        <div
          ...
          <div className="text-white pointer-events-none absolute flex w-full flex-col items-center transition-[bottom] bottom-0 p-4">
            <div className={props.fullscreen ? "" : "transform origin-bottom text-[0.5rem]"}>
              <CaptionCue text={t("settings.subtitles.previewQuote") ?? undefined} styling={props.styling} overrideCasing={false} />
            </div>
          </div>
        </div>
      </Transition>
...
export function CaptionsPart(props: { styling: SubtitleStyling; setStyling: (s: SubtitleStyling) => void }) {
  const { t } = useTranslation();
      ...
      <div className="grid md:grid-cols-[1fr,356px] gap-8">
        <div className="space-y-6">
          <CaptionSetting
            ...
            max={100}
            min={0}
            onChange={(v) => props.setStyling({ ...props.styling, backgroundOpacity: v / 100 })}
          ...
          <CaptionSetting
            ...
            max={100}
            min={0}
            onChange={(v) => props.setStyling({ ...props.styling, backgroundBlur: v / 100 })}
          ...
          <CaptionSetting
            ...
            min={1}
            textTransformer={(s) => `${s}%`}
            onChange={(v) => props.setStyling({ ...props.styling, size: v / 100 })}
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex justify-center items-center">
              {colors.map((v) => (
                <ColorOption onClick={() => props.setStyling({ ...props.styling, color: v })} color={v} active={props.styling.color === v} key={v} />
              ))}
            </div>
          </div>
        </div>
        <CaptionPreview show styling={props.styling} onToggle={() => setFullscreenPreview((s) => !s)} />
        <CaptionPreview show={fullscreenPreview} fullscreen styling={props.styling} onToggle={() => setFullscreenPreview((s) => !s)} />
      </div>
    </div>
  );
}
